...
type
  TfrmQRResumenDECIsMant = class(TForm)
    ...
    QRLabel5: TQRLabel;
    QRDBTextIMPORTE_EFEC: TQRDBText;
...
begin
  ...
  Script := 'SELECT EDIFICIO, SUM(IMPORTE_EFEC) IMPORTE_EFEC, ' +
            'SUM(IMPORTE_RAE) IMPORTE_RAE, ' + 'SUM(IMPORTE_CHEQUE) IMPORTE_CHEQUE,' +
            'SUM(IMPORTE_EFEC+IMPORTE_RAE+IMPORTE_CHEQUE) TOTAL ' +
            'FROM VW_RESUMEN_DECI_MANT2 ' +
