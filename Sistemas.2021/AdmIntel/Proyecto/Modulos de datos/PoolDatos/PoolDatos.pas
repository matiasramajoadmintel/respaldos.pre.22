...
uses
  SysUtils, Classes, DB, DBTables, DateUtils, Variants, QForms, Controls, ElecCoef;
...
type
  TdmDatos = class(TDataModule)
    ...
    COEFPORCENTAJE2: TFloatField;
    qryOcupParticulares: TQuery;
    qryOcupParticularesIDOCUPANTE: TIntegerField;
    qryOcupParticularesOCUPANTE: TStringField;
    qryOcupParticularesLOCACION: TStringField;
    qryOcupParticularesFECHAINI: TDateTimeField;
    qryOcupParticularesFECHAFIN: TDateTimeField;
    qryOcupParticularesIDEDIFICIO: TIntegerField;
    qryOcupParticularesIDDEPARTAMENTO: TIntegerField;
    qryOcupParticularesLEGAJO: TIntegerField;
    qryOcupParticularesSUELDO: TFloatField;
    tblOcupanteParticular_: TTable;
    tblOcupanteParticular_IDPERSONA: TIntegerField;
    tblOcupanteParticular_IDGARANTE: TIntegerField;
    tblOcupanteParticular_PARENTEZCO: TStringField;
    tblOcupanteParticular_COSTOALQUILER: TFloatField;
  ...
  private
    ...
    procedure GenerarDescuentosAlquilerParticulares(Mes, Anio: Word; var CantActivos: Integer; var Monto: Currency);
...
//  GenerarDescuentosAlquilerParticulares(Mes, Anio, CantActivosCiv, MontoCiv);
...
var
  DivisorRenta, Alquiler, Coeficiente, Coeficiente2, Coeficiente3, Coeficiente4, FondoReserva, Sueldo, Cod202, Cod218: Real;
  AntGrado: Byte;
  M: String;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Coeficiente2 := COEFPORCENTAJE2.Value;
        ...
        begin
          ...
          //esto parece estar de mas, la variable Alquiler no parece usarse nuevamente
          Alquiler := ((Coeficiente + Coeficiente2) * Sueldo)/100 +
                      qryOcupMilitaresRENTAPROPIA.Value / DivisorRenta;

          //Agrega los registros de descuentos, 418, 375, Fondo Reserva
         // Aun queda por definir las variables para 0.17 y 0.83, por ahora estan en el codigo sin la variable
          DESC.Append;
          DESCIDOCUPANTE.Value := qryOcupMilitaresIDPERSONA.Value;
          DESCIDTIPODESCUENTO.Value := 1; //418 Alquiler / 01
          DESCMES.Value := M;
          DESCANIO.Value := Anio;
          DESCMONTO.Value := (Sueldo * Coeficiente * 0.83)/100; //Antes se calculaba como Alquiler * 0.9;
          DESC.Post;

          DESC.Append;
          DESCIDOCUPANTE.Value := qryOcupMilitaresIDPERSONA.Value;
          DESCIDTIPODESCUENTO.Value := 2; //375 RAE / 01
          DESCMES.Value := M;
          DESCANIO.Value := Anio;
          DESCMONTO.Value :=  (Sueldo * Coeficiente * 0.17 )/100; // Antes se calculaba como Alquiler * 0.1;
          DESC.Post;

          // Este nuevo valor se agrega en abril 2018
          DESC.Append;
          DESCIDOCUPANTE.Value := qryOcupMilitaresIDPERSONA.Value;
          DESCIDTIPODESCUENTO.Value := 4; //Fondo Reserva / 01
          DESCMES.Value := M;
          DESCANIO.Value := Anio;
          DESCMONTO.Value := (Sueldo * Coeficiente2)/100; // Antes no existia este valor
          DESC.Post;

          CantActivos := CantActivos + 1;
          Monto := Monto + (Sueldo * Coeficiente * 0.83)/100; // antes era Monto := Monto + Alquiler * 0.9;


{
          //viejo codigo previo a abril 2018
          //Calcula el monto del alquiler.
...
var
  Alquiler, Coeficiente,Coeficiente2, Coeficiente3, Coeficiente4, Sueldo: Real;
  M: String;
//Lista de los ocupantes a los que no se les pudo encontrar un coeficiente de alquiler.
  SinCoeficiente: TStrings;
  CantAmb: Byte;
begin
 ...
 {        //viejo codigo previo a abril 2018
          ...
          //Agrega los registros de descuentos, 418, 375
          DESC.Append;
          DESCIDOCUPANTE.Value := qryOcupCivilesIDOCUPANTE.Value;
          DESCIDTIPODESCUENTO.Value := 1; //418 / 01
          DESCMES.Value := M;
...
procedure TdmDatos.GenerarDescuentosAlquilerParticulares(Mes, Anio: Word; var CantActivos: Integer; var Monto: Currency);

var
  Alquiler, Coeficiente,Coeficiente2, Sueldo: Real;
  M: String;
//Lista de los ocupantes a los que no se les pudo encontrar un coeficiente de alquiler.
  SinCoeficiente: TStrings;
  CantAmb: Byte;
begin
  ...
  try
    ...
    DESC.Open;
    qryOcupParticulares.Open;
    qryOcupParticulares.First;
    while not qryOcupParticulares.Eof do
    begin
      Existe(DEPTO, 'IDDEPARTAMENTO', qryOcupParticularesIDDEPARTAMENTO.Value);
      {Ubica el registro en la tabla de coeficientes para obtener el que corresponde.
       Si no lo encuentra agrega una fila a la lista de ocupantes sin coeficiente}
      if not Existe(COEF,'TIPO','PERSONAL','CANT_AMB',
                VarArrayOf(['DEPTO', 'CIV', DEPTOCANTAMB.Value])) then
        SinCoeficiente.Add(qryOcupParticularesLEGAJO.Text + ' ; PC ' + ' DEPTO ' +
                           DEPTOCANTAMB.Text)
      else
      begin
       {if (qryOcupCivilesIDOCUPANTE.Value = 434) then
        begin
          ...
          Existe(tblOcupanteParticular_, 'IDPERSONA', qryOcupParticularesIDOCUPANTE.Value);
          Sueldo := tblOcupanteParticular_COSTOALQUILER.Value;
         ...
         // Aun queda por definir las variables para 0.17 y 0.83, por ahora estan en el codigo sin la variable
          DESC.Append;
          DESCIDOCUPANTE.Value := qryOcupParticularesIDOCUPANTE.Value;
          DESCIDTIPODESCUENTO.Value := 1; //418 Alquiler / 01
          DESCMES.Value := M;
          DESCANIO.Value := Anio;
          DESCMONTO.Value := tblOcupanteParticular_COSTOALQUILER.Value; //(Sueldo * Coeficiente)/100; //Antes se calculaba como Alquiler * 0.9;
          DESC.Post;
          ...
          DESCMES.Value := M;
          DESCANIO.Value := Anio;
          DESCMONTO.Value :=  ((Sueldo * Coeficiente2)/100) * 0.17; // Antes se calculaba como Alquiler * 0.1;
          DESC.Post;
          ...
          DESCMES.Value := M;
          DESCANIO.Value := Anio;
          DESCMONTO.Value := (Sueldo * Coeficiente2)/100 * 0.83; // Antes no existia este valor
          DESC.Post;

          CantActivos := CantActivos + 1;
          Monto := Monto + (Sueldo * Coeficiente)/100; // antes era Monto := Monto + Alquiler * 0.9;
          end;


        qryOcupParticulares.Next;
    end;
    except on EDatabaseError do
    begin
      MsjError('Se produjo un error generando los descuentos por alquiler');
