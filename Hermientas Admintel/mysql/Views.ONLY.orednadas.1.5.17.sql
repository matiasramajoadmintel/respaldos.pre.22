...
-- Dumping structure for view zzz.vw_cant_activos_x_edif
-- Removing temporary table and create final VIEW structure
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `vw_pagos` AS SELECT GASTO_AD.IDGASTO_AD, GASTO_AD.IDPROVEEDOR, Proveedor.RAZON_SOCIAL, SUM( Gasto_ad.COSTO )
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `vw_pagos_efectuados` AS SELECT Pago.IDPAGO, Pago.FECHA_PAGO, Pago.IMPORTE, Pago.TIPO_PAGO, Pago.NRO_CHEQUE, Pago.COBRADO, Proveedor.RAZON_SOCIAL, Personal.APELLIDO || ' ' || Personal.NOMBRE
FROM PAGO Pago
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `vw_trab_pagados` AS SELECT Trabajo.IDTRABAJO, Trabajo.IDPAGO,   ( ('Factura Nº ' || Trabajo.NRO_FACT ) || ' - ')  || Categoria.DESCRIP , Trabajo.COSTO, Trabajo.FECHA_FIN, DEPARTAMENTO.IDEDIFICIO, Trabajo.TIPO_TRAB, Trabajo.NRO_DECI, TRABAJO.NRO_ORDEN, Proveedor.RAZON_SOCIAL, Pago.TIPO_PAGO, Pago.NRO_CHEQUE
...
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `vw_trab_tareas`;
...
/*!40014 SET FOREIGN_KEY_CHECKS=IF(@OLD_FOREIGN_KEY_CHECKS IS NULL,'ON', @OLD_FOREIGN_KEY_CHECKS) */;
