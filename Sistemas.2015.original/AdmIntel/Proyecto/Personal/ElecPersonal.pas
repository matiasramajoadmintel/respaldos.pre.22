...
//este script no esta funcionando, no estaba programado, lo agregue yo pero no funciona
 {    Script := 'SELECT * FROM PERSONAL';

Filtro := '';
     if edtDesc.Text <> ''  then
       Filtro := 'WHERE (' +
                 'NOMBRE' + #39 + '%' + Uppercase(edtDesc.Text) + '%' + #39 + ') OR (' +
                 'APELLIDO' + #39 + '%' + UpperCase(edtDesc.Text) + '%' + #39 + ')';    }
     with qryPers do
     begin
          Close;
{          SQL.Clear;
