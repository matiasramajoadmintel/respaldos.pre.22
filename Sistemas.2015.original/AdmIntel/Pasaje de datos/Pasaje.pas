...
type
  TForm1 = class(TForm)
    ...
    GradoABREVIATURA: TIBStringField;
    OcupaNuevoANTSERVICIO: TIntegerField;
    OcupaNuevoTIPOPERSONAL: TIBStringField;
...
begin
  ...
  EdificioN.Close;
  EdificioV.Close;
  MsjInfo(PChar('Se migraron ' + IntToStr(i) + ' edificios'));
end;

procedure TForm1.btnPersonasClick(Sender: TObject);
var
  Todo, Apellido, Nombre: String;
  i, j, IdPersona, IdDepartamento: Word;
begin
  Persona.Open;
  Ocupante.Open;
  DeptosViejos.Open;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Existe(Grado,'ABREVIATURA',OcupanteGrado.Text);
  ...
  OcupaNuevo.Close;
  Ocupacion.Close;
  MsjInfo(PChar('Se migraron ' + IntToStr(j) + ' ocupantes'));
end;
...
begin
  ...
  Especialidad.Close;
  MsjInfo(PChar('Se migraron ' + IntToStr(i) + ' especialidades'));
end;
