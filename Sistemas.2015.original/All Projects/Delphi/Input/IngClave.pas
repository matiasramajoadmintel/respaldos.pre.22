...
type
  ...
  public
    ...
    Function ObtenerByte(var Nro:Byte;const Titulo:String):Boolean;
...
Function TfrmIngClave.ObtenerByte(var Nro:Byte;const Titulo:String):Boolean;
begin
  Result := False;
  Tipo := 0;
  Caption := Titulo;
  if Nro = 0 then edtCod.Text := ''
     else edtCod.Text := IntToStr(Nro);
  ActiveControl := edtCod;
  ...
  begin
    ...
    if Result then
    begin
      Nro := StrToInt(edtCod.Text);
      if (Nro <> Trunc(Nro)) or (Nro < 0) or (Nro > 255) then
      begin
        MsjError('Debe ingresar un numero entero entre 0 y 255');
        Result := False;
      end;
    end;
  end;
