...
type
  TfrmEstGastosAdmin = class(TForm)
...
var
  frmEstGastosAdmin: TfrmEstGastosAdmin;
...
procedure TfrmEstGastosAdmin.Mostrar(FechaInicio, FechaFin: TDateTime);
begin
...
procedure TfrmEstGastosAdmin.PrepararFormulario(FechaInicio, FechaFin: TDateTime);
begin
...
procedure TfrmEstGastosAdmin.FiltrarQuery(FechaInicio, FechaFin: TDateTime);
var
  Filtro: String;
begin
  Filtro := '';
  ...
  //Filtra por fechas
    if Filtro = '' then
      Filtro := 'FECHA_PAGO >= ' + #39 + DateToStr(FechaInicio) + #39 +
                'AND FECHA_PAGO <= ' + #39 + DateToStr(FechaFin) + #39
    else Filtro := Filtro + ' AND FECHA_PAGO >= ' + #39 + DateToStr(FechaInicio) + #39 +
                   'AND FECHA_PAGO <= ' + #39 + DateToStr(FechaFin) + #39;
...
procedure TfrmEstGastosAdmin.FormCreate(Sender: TObject);
begin
  RecuperarEstadoVentana(Name,Self);
end;

procedure TfrmEstGastosAdmin.FormDestroy(Sender: TObject);
begin
  GuardarEstadoVentana(Name,Self);
end;

procedure TfrmEstGastosAdmin.ComboClick(Sender: TObject);
begin
  FiltrarQuery(dtpIni.DateTime, dtpFin.DateTime);
end;
