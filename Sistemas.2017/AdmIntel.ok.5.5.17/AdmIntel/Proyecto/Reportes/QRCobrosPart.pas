...
begin
  ...
  // Crea una aplicacion Excel.
    ...
    Libro.Cells [i+6,4] := Format('%.3f', [qryCobrosPart.FieldByName ('ALQUILER').AsFloat]);
    Libro.Cells [i+6,5] := Format('%.3f', [qryCobrosPart.FieldByName ('RAE').AsFloat]);
    Libro.Cells [i+6,6] := Format('%.4f', [qryCobrosPart.FieldByName ('RAE').AsCurrency  + qryCobrosPart.FieldByName('ALQUILER').AsCurrency ]);
    Libro.Cells [i+6,7] := Format('%.3f', [qryCobrosPart.FieldByName ('EXPENSAS').AsFloat]);
    Libro.Cells [i+6,8] := Format('%.3f', [qryCobrosPart.FieldByName ('EXPENSAS').AsFloat + qryCobrosPart.FieldByName ('RAE').AsFloat + qryCobrosPart.FieldByName ('ALQUILER').AsFloat]);
