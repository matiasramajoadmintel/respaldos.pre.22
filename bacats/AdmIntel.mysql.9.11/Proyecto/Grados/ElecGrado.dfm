object frmElecGrado: TfrmElecGrado
  Left = 782
  Top = 182
  Width = 727
  Height = 575
  Caption = 'Grado'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 711
    Height = 45
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Label1: TLabel
      Left = 10
      Top = 5
      Width = 81
      Height = 13
      Caption = 'Nombre a buscar'
    end
    object edtDesc: TEdit
      Left = 10
      Top = 19
      Width = 275
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnKeyPress = edtDescKeyPress
    end
    object btnBuscar: TBitBtn
      Left = 286
      Top = 19
      Width = 75
      Height = 21
      Caption = 'Buscar'
      TabOrder = 1
      OnClick = btnBuscarClick
    end
  end
  object Panel4: TPanel
    Left = 0
    Top = 482
    Width = 711
    Height = 25
    Align = alBottom
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    object btnAgregar: TSpeedButton
      Left = 4
      Top = 2
      Width = 60
      Height = 22
      Hint = 'o presione la tecla "A"'
      Caption = '&Agregar'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = btnAgregarClick
    end
    object btnEliminar: TSpeedButton
      Left = 65
      Top = 2
      Width = 60
      Height = 22
      Hint = 'o presione la tecla "Delete"'
      Caption = 'E&liminar'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = btnEliminarClick
    end
    object btnModificar: TSpeedButton
      Left = 123
      Top = 2
      Width = 65
      Height = 22
      Hint = 'o presione la tecla "Enter"'
      Caption = '&Modificar'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = btnModificarClick
    end
    object btnRespaldar: TSpeedButton
      Left = 188
      Top = 2
      Width = 71
      Height = 22
      Caption = '&Respaldar'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = btnRespaldarClick
    end
    object btnConsultar: TSpeedButton
      Left = 257
      Top = 2
      Width = 69
      Height = 22
      Caption = '&Consultar'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
      OnClick = btnConsultarClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 507
    Width = 711
    Height = 30
    Align = alBottom
    TabOrder = 2
    object Panel5: TPanel
      Left = 525
      Top = 1
      Width = 185
      Height = 28
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 0
      object btnCerrar: TBitBtn
        Left = 108
        Top = 2
        Width = 75
        Height = 25
        Cancel = True
        Caption = '&Cerrar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ModalResult = 2
        ParentFont = False
        TabOrder = 1
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
      object btnElegir: TBitBtn
        Left = 32
        Top = 2
        Width = 75
        Height = 25
        Caption = '&Elegir'
        Default = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ModalResult = 1
        ParentFont = False
        TabOrder = 0
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 45
    Width = 711
    Height = 437
    Align = alClient
    BevelInner = bvLowered
    BorderWidth = 3
    Caption = 'Panel1'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    object grdDatos: TDBGrid
      Left = 5
      Top = 5
      Width = 701
      Height = 427
      Align = alClient
      BorderStyle = bsNone
      DataSource = dsGrado
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -15
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnKeyDown = grdDatosKeyDown
      OnTitleClick = grdDatosTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'descripcion'
          Title.Alignment = taCenter
          Title.Caption = 'Descripci'#243'n'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = []
          Width = 207
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'abreviatura'
          Title.Alignment = taCenter
          Title.Caption = 'Abreviatura'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'tiempo_minimo'
          Title.Alignment = taCenter
          Title.Caption = 'Tiempo m'#237'nimo'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'cod201'
          Title.Alignment = taCenter
          Title.Caption = 'Sueldo'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'cod209'
          Title.Alignment = taCenter
          Title.Caption = 'R.G.A.S.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'orden'
          Title.Alignment = taCenter
          Title.Caption = 'Orden'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = []
          Visible = True
        end>
    end
  end
  object dsGrado: TDataSource
    DataSet = ZqryGrados
    Left = 602
    Top = 83
  end
  object qryGrados: TQuery
    DatabaseName = 'ADB'
    SQL.Strings = (
      'select * from GRADO')
    Left = 564
    Top = 164
    object qryGradosIDGRADO: TIntegerField
      FieldName = 'IDGRADO'
      Origin = 'DB.GRADO.IDGRADO'
    end
    object qryGradosDESCRIPCION: TStringField
      FieldName = 'DESCRIPCION'
      Origin = 'DB.GRADO.DESCRIPCION'
      Size = 50
    end
    object qryGradosABREVIATURA: TStringField
      FieldName = 'ABREVIATURA'
      Origin = 'DB.GRADO.ABREVIATURA'
      Size = 2
    end
    object qryGradosTIEMPO_MINIMO: TIntegerField
      FieldName = 'TIEMPO_MINIMO'
      Origin = 'DB.GRADO.TIEMPO_MINIMO'
    end
    object qryGradosCOD201: TFloatField
      FieldName = 'COD201'
      Origin = 'DB.GRADO.COD201'
      currency = True
    end
    object qryGradosCOD209: TFloatField
      FieldName = 'COD209'
      Origin = 'DB.GRADO.COD209'
      currency = True
    end
    object qryGradosORDEN: TIntegerField
      FieldName = 'ORDEN'
      Origin = 'ADB.GRADO.ORDEN'
    end
  end
  object tbl1: TTable
    DatabaseName = 'DBAdmin'
    TableName = 'GRADO'
    Left = 568
    Top = 125
  end
  object tbl2: TTable
    DatabaseName = 'DBAdmin'
    TableName = 'RESPALDO_GRADO'
    Left = 568
    Top = 93
  end
  object bm1: TBatchMove
    Destination = tbl2
    Source = tbl1
    Left = 568
    Top = 61
  end
  object qryCheques: TQuery
    AutoCalcFields = False
    DatabaseName = 'ADB'
    SQL.Strings = (
      'SELECT DISTINCT *'
      'FROM VW_CHEQUES_EMITIDOS')
    Left = 452
    Top = 10
    object qryChequesFECHA: TDateTimeField
      FieldName = 'FECHA'
      Origin = 'ADB.VW_CHEQUES_EMITIDOS.FECHA'
    end
    object qryChequesPROVEEDOR: TStringField
      FieldName = 'PROVEEDOR'
      Origin = 'ADB.VW_CHEQUES_EMITIDOS.PROVEEDOR'
      Size = 45
    end
    object qryChequesIMPORTE: TFloatField
      FieldName = 'IMPORTE'
      Origin = 'ADB.VW_CHEQUES_EMITIDOS.IMPORTE'
      DisplayFormat = '#,0.00'
    end
    object qryChequesNROCHEQUE: TStringField
      FieldName = 'NROCHEQUE'
      Origin = 'ADB.VW_CHEQUES_EMITIDOS.NROCHEQUE'
    end
    object qryChequesCOBRADO: TStringField
      FieldName = 'COBRADO'
      Origin = 'ADB.VW_CHEQUES_EMITIDOS.COBRADO'
      Size = 2
    end
    object qryChequesIDPAGO: TIntegerField
      FieldName = 'IDPAGO'
      Origin = 'ADB.VW_CHEQUES_EMITIDOS.IDPAGO'
    end
  end
  object ZConnection1: TZConnection
    Protocol = 'mysql-3.20'
    Port = 3306
    Database = 'admintel_test'
    User = 'root'
    Catalog = 'admintel_test'
    AutoCommit = True
    ReadOnly = False
    TransactIsolationLevel = tiNone
    Connected = True
    SQLHourGlass = False
    Left = 504
    Top = 5
  end
  object ztbl1: TZTable
    Connection = ZConnection1
    RequestLive = False
    CachedUpdates = False
    Active = True
    TableName = 'grado'
    ShowRecordTypes = [utUnmodified, utModified, utInserted]
    MasterSource = dsGrado
    UpdateMode = umUpdateChanged
    WhereMode = wmWhereKeyOnly
    Options = [doCalcDefaults]
    Left = 600
    Top = 125
    object ztbl1idgrado: TIntegerField
      FieldName = 'idgrado'
      Required = True
    end
    object ztbl1descripcion: TStringField
      FieldName = 'descripcion'
      Required = True
      Size = 150
    end
    object ztbl1abreviatura: TStringField
      FieldName = 'abreviatura'
      Required = True
      Size = 6
    end
    object ztbl1tiempo_minimo: TIntegerField
      FieldName = 'tiempo_minimo'
    end
    object ztbl1cod201: TFloatField
      FieldName = 'cod201'
    end
    object ztbl1cod209: TFloatField
      FieldName = 'cod209'
    end
    object ztbl1orden: TIntegerField
      FieldName = 'orden'
    end
  end
  object ZqryGrados: TZQuery
    Connection = frmPrincipal.masterkey
    RequestLive = True
    CachedUpdates = False
    Active = True
    SQL.Strings = (
      'select * from GRADO')
    ParamCheck = True
    Params = <
      item
        DataType = ftInteger
        Name = 'idgrado'
        ParamType = ptUnknown
      end>
    ShowRecordTypes = [utUnmodified, utModified, utInserted]
    UpdateMode = umUpdateChanged
    WhereMode = wmWhereKeyOnly
    Options = [doCalcDefaults]
    Left = 600
    Top = 165
    ParamData = <
      item
        DataType = ftInteger
        Name = 'idgrado'
        ParamType = ptUnknown
      end>
    object ZqryGradosidgrado: TIntegerField
      FieldName = 'idgrado'
      Required = True
    end
    object ZqryGradosdescripcion: TStringField
      FieldName = 'descripcion'
      Required = True
      Size = 150
    end
    object ZqryGradosabreviatura: TStringField
      FieldName = 'abreviatura'
      Required = True
      Size = 6
    end
    object ZqryGradostiempo_minimo: TIntegerField
      FieldName = 'tiempo_minimo'
    end
    object ZqryGradoscod201: TFloatField
      FieldName = 'cod201'
    end
    object ZqryGradoscod209: TFloatField
      FieldName = 'cod209'
    end
    object ZqryGradosorden: TIntegerField
      FieldName = 'orden'
    end
  end
  object ZQuery1: TZQuery
    RequestLive = False
    CachedUpdates = False
    ParamCheck = True
    Params = <>
    ShowRecordTypes = [utUnmodified, utModified, utInserted]
    UpdateMode = umUpdateChanged
    WhereMode = wmWhereKeyOnly
    Options = [doCalcDefaults]
    Left = 408
    Top = 8
  end
end
