...
uses
  ...
  IBCustomDataSet, IBTable, Principal, ZAbstractRODataset,
...
type
  TfrmOcupante = class(TForm)
    ...
    ztblPER_nrodocumento: TIntegerField;
    ztblPER_tipodocumento: TStringField;
    ztblPER_nombre: TStringField;
    ztblPER_apellido: TStringField;
    ztblPER_telefono: TStringField;
    ztblPER_email: TStringField;
    ztblPER_tipo: TStringField;
    ztblPER_obs: TBlobField;
...
begin
  ...
  begin
    ztblPER_.Open;
    ztblPER_.Append;
    ztblPER_TIPO.Text := OCUP_MIL;
  end
  else if gModo = MODE_UPDATE then //Modificación
  begin
    gTipoAnterior := ztblPER_TIPO.Text;
    ztblPER_.Edit;
  end;
...
begin
  ...
  begin
    ztblPER_.Last;
    IdPersona := ztblPER_IDPERSONA.Value
  end;
end;

function TfrmOcupante.Modificar(IdPersona: Integer; FormSiguiente: TForm): Boolean;
begin
  if not Existe(ztblPER_, 'IDPERSONA', IdPersona) then
  begin
...
begin
  ...
  if ztblPER_TIPODOCUMENTO.IsNull or ztblPER_NRODOCUMENTO.IsNull then
  begin
...
begin
  ...
  begin
    ...
    begin
      if ztblPER_TIPO.Value = OCUP_MIL then
      begin
      ...
      if ztblPER_TIPO.Value = OCUP_CIV then
      begin
      ...
      if ztblPER_TIPO.Value = OCUP_PART then
      begin
      ...
      if ztblPER_TIPO.Value = MIL_NO_OCUP then
      begin
        Self._FormSiguiente := TfrmMilitarNoOcupante.Create(nil);
        TfrmMilitarNoOcupante(Self._FormSiguiente).gModo := Self.gModo;
        TfrmMilitarNoOcupante(Self._FormSiguiente).Mostrar(IdPersona);       // aca me tira erro de database conection not ready
      end;
    end
    else
...
begin
  ztblPER_IDPERSONA.Value := dmStoreProc.getId('IDPERSONA') + 1;
end;
