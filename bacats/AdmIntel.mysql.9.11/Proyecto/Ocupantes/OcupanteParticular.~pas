...
type
  TfrmOcupanteParticular = class(TForm)
    ...
    ztblOP_idpersona: TIntegerField;
    ztblOP_idgarante: TIntegerField;
    ztblOP_parentezco: TStringField;
    ztblOP_costoalquiler: TFloatField;
...
begin
  ...
  begin
    if not Existe(ztblOP_, 'IDPERSONA', IdPersona) then
      gModo := MODE_INSERT
    else
    begin
      ztblOP_.Edit;
      Existe(ztblPER_, 'IDPERSONA', ztblOP_IDGARANTE.Value);
      edtGarante.Text := ztblPER_APELLIDO.Text + ', ' + ztblPER_NOMBRE.Text;
    end;
  end;
  ...
  begin
    ztblOP_.Open;
    ztblOP_.Append;
    ztblOP_IDPERSONA.Value := IdPersona;
    edtGarante.Text := '';
  end;
...
begin
  try
    Existe(ztblOP_, 'IDPERSONA', IdPersona);
    ztblOP_.Delete;
    Result := True;
...
begin
  ...
  begin
    ...
    begin
      ztblOP_IDGARANTE.Value := IdOcupante;
