...
uses
  ...
  Dialogs, Buttons, StdCtrls, ExtCtrls, Grids, DBGrids, DB, DBTables, Principal,
  IBCustomDataSet, IBTable, IBQuery, ZAbstractRODataset, ZAbstractDataset,
  ZDataset;
...
type
  TfrmElecEspecialidad = class(TForm)
    ...
    ZQuery1: TZQuery;
    ZQuery1idespecialidad: TIntegerField;
    ZQuery1descripcion: TStringField;
    ZQuery1abrev: TStringField;
...
begin
        ...
        begin
                Descripcion:=ZQuery1Descripcion.Value;
                Result:=ZQuery1IdEspecialidad.Value;
        end
        else
                Result:=0;
end;
...
begin
        ...
        begin
                PrepararQuery (gOrden);
                Existe (ZQuery1,'IdEspecialidad',IdEspecialidad);
        end;
end;
...
begin
        with TfrmIngEspecialidad.Create(nil) do
        begin
                R:=Eliminar(ZQuery1IdEspecialidad.Value);
                Release;
        end;
        if R then
                PrepararQuery (gOrden);
...
var
        ...
        IdEspecialidad:integer;
begin
        IdEspecialidad:=ZQuery1IdEspecialidad.Value;
        with TfrmIngEspecialidad.Create(nil) do
        begin
...
var
  F: TField;
begin
        F := ZQuery1.FindField(Column.FieldName);
