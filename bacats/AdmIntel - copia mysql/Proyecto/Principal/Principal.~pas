...
uses
  ...
  ImgList, DBCtrls, Buttons, DBTables, scExcelExport, ZConnection;
...
type
  TfrmPrincipal = class(TForm)
    ...
    Personal1: TMenuItem;
    Pagosporpagador1: TMenuItem;
    Formulariodepagos1: TMenuItem;
    N11: TMenuItem;
    ListadoPermisionariosTitulares1: TMenuItem;
    Personalsuperior1: TMenuItem;
    Personalsubalterno1: TMenuItem;
    Personalcivil1: TMenuItem;
    Remanentes1: TMenuItem;
    pnlFiltroDesc: TPanel;
    Panel4: TPanel;
    ...
    Label11: TLabel;
    N12: TMenuItem;
    Generacinexpensasdeptosvacios1: TMenuItem;
    rgVacios: TRadioGroup;
    Gastosadministrativos2: TMenuItem;
    Gastosfinancieros1: TMenuItem;
    N13: TMenuItem;
    N14: TMenuItem;
    ResumenDECIsMantenimiento1: TMenuItem;
    ResumenDECIsServicios1: TMenuItem;
    Deudores1: TMenuItem;
    Listadodedeudores1: TMenuItem;
    scExcelExport1: TscExcelExport;
    btn1: TBitBtn;
    masterkey: TZConnection;
    ...
    procedure Pagosporpagador1Click(Sender: TObject);
    procedure Formulariodepagos1Click(Sender: TObject);
    procedure rgPagadoClick(Sender: TObject);
    procedure Personalsuperior1Click(Sender: TObject);
    procedure Personalsubalterno1Click(Sender: TObject);
    procedure Personalcivil1Click(Sender: TObject);
    procedure Remanentes1Click(Sender: TObject);
    procedure Generacinexpensasdeptosvacios1Click(Sender: TObject);
    procedure Gastosadministrativos2Click(Sender: TObject);
    procedure ResumenDECIsMantenimiento1Click(Sender: TObject);
    procedure ResumenDECIsServicios1Click(Sender: TObject);
    procedure Deudores1Click(Sender: TObject);
    procedure Listadodedeudores1Click(Sender: TObject);
    procedure btn1Click(Sender: TObject);
    procedure Gastosfinancieros1Click(Sender: TObject);
  ...
  private
    ...
    gOrden: String;
    gId: Longword;
    ListadosConABM: array[1..20] of Boolean;
    ...
    procedure CargarListadosConABM;
    procedure AplicarFiltroGral;
    procedure AplicarFiltroDescuento;
...
uses Trabajo, Comunes, ElecEnt, ElecProv, Proveedor, ElecMed, PoolQuerys,
  ...
  QRCobrosMil, QRCobrosCiv, IngIngreso, Personal, QRPagosPorPagador,
  QRFormularioPagos, ElecPersonal, IngDescuento, QRTitulares,
  QRTitularesRemanentes, GastoAd, Est_GastosAdmin, QRResumenDECIsMant,
  QRResumenDECIsServ, QRDeudores, Est_GastosFin;
...
begin
  ...
  if (not pnlFiltro.Visible and (gListado <> DESCUENTOS)) then
  begin
    ...
    pnlFiltroDesc.Visible := False;
  end;

  if (gListado = DESCUENTOS) then
  begin
    pnlFiltro.Visible := false;
    pnlFiltroDesc.Visible := true;
  end;
  ...
  rgPagado.Enabled := (gListado <> PAGOS_EFECTUADOS);
  rgTipoPago.Enabled := (gListado = PAGOS_EFECTUADOS);
...
begin
  pnlBotones.Visible := ListadosConABM[gListado];

  btnAgregar.Enabled := ListadosConABM[gListado];

  btnEliminar.Enabled := (ListadosConABM[gListado]) and
                         (dsDatos.DataSet.RecordCount > 0);

  btnModificar.Enabled := (ListadosConABM[gListado]) and
                         (dsDatos.DataSet.RecordCount > 0);

  btnDetalle.Enabled := ((gListado = TRAB_DEPTO) or (gListado = TRAB_EDIF) or
                        (gListado = PAGOS_EFECTUADOS) or (gListado = INGRESOS)) and
                        (dsDatos.DataSet.RecordCount > 0);
...
begin
  ...
  case Consulta of
    ...
    PAGOS_EFECTUADOS: pnlTitulo.Caption  := 'Pagos Efectuados';
    DESCUENTOS: pnlTitulo.Caption := 'Listado de Descuentos';
    DEUDORES: pnlTitulo.Caption := 'Listado de Deudores'
  end;
...
begin
  ...
  pnlTitulo.Caption := 'Listado de ocupantes históricos';
...
begin
  ...
  if dsDatos.DataSet.FindField('IMPORTE') <> nil then
    NombreCampo := 'IMPORTE'
  else
  if dsDatos.DataSet.FindField('MONTO') <> nil then
    NombreCampo := 'MONTO'
  else
    NombreCampo := '';
  ...
  if NombreCampo <> '' then
  begin
    Total1 := SumaColumna(dsDatos.DataSet, NombreCampo);
...
procedure TfrmPrincipal.CargarListadosConABM;

begin
  ListadosConABM[1] := True;   //INGRESOS
  ListadosConABM[2] := False;  //COBROS_PART
  ListadosConABM[3] := False;  //RESUMEN_COBROS
  ListadosConABM[4] := True;   //DESCUENTOS
  ListadosConABM[5] := False;  //OCUPANTES_ACTUALES
  ListadosConABM[6] := False;  //OCUPANTES_HISTORICOS
  ListadosConABM[7] := True;   //PAGOS_EFECTUADOS
  ListadosConABM[8] := True;   //GASTOS_ADMIN
  ListadosConABM[9] := True;   //GASTOS_FINAN
  ListadosConABM[10] := True;  //TRAB_DEPTO
  ListadosConABM[11] := True;  //TRAB_EDIF
  ListadosConABM[12] := True;  //GASTOS_SERV
  ListadosConABM[13] := True;  //RECIBOS_EMITIDOS
  ListadosConABM[14] := False; //DET_TAREAS
  ListadosConABM[15] := False; //RES_TAREAS_DEPTO
  ListadosConABM[16] := False; //RES_TAREAS_GRAL
  ListadosConABM[17] := True;  //MAT_USADOS
  ListadosConABM[18] := False;
  ListadosConABM[19] := False;
  ListadosConABM[20] := False;
end;
...
begin
  ...
  if gListado = PAGOS_EFECTUADOS then
  begin
...
procedure TfrmPrincipal.AplicarFiltroGral;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
       ...
       //Filtra por tipo de pago
        ...
        else if rgTipoPago.ItemIndex = 2 then //RAE
        begin
          if Filter <> '' then
            Filter := Filter + 'AND TIPO_PAGO = ' + #39 + 'R' + #39
          else Filter := 'TIPO_PAGO = ' + #39 + 'R' + #39;
        end;
      end
      else
    ...
    //Filtra por Vacios
    if (gListado = GASTOS_SERV) then
    begin
      if rgVacios.ItemIndex = 0 then //Sí
      begin
        if (Filter <> '') then
          Filter := Filter + 'AND Vacios = ' + #39 + 'Sí' + #39
        else Filter := 'Vacios = ' + #39 + 'Sí' + #39;
      end;
      if rgVacios.ItemIndex = 1 then //Sí
      begin
        if (Filter <> '') then
          Filter := Filter + 'AND Vacios = ' + #39 + 'No' + #39
        else Filter := 'Vacios = ' + #39 + 'No' + #39;
      end;
    end;
...
procedure TfrmPrincipal.AplicarFiltroDescuento;
var
  Ocup, Filtro: String;
begin
  with dsDatos,DataSet do
  begin
    ...
    if (edtOcupante.Text <> '') then
    begin
      Ocup := UpperCase(edtOcupante.Text);
      Filtro :=  '((APENOM = ' + #39 + '*' + Ocup + #39 + ') OR (' +
                 'APENOM = ' + #39 + '*' + Ocup + '*' + #39 + ') OR (' +
                 'APENOM = ' + #39 + Ocup + '*' + #39 + ') OR (' +
                 'APENOM = ' + #39 + Ocup + #39 + '))';
    end;
    ...
    begin
      if Filtro = '' then
        Filtro := 'DESCRIPCION = ' + #39 + cmbTipoDescuento.Text + #39
      else
        Filtro := Filtro + 'AND (DESCRIPCION = ' + #39 + cmbTipoDescuento.Text + #39 + ')';
    end;

    if (cmbMes.Text <> '') then
    begin
      if Filtro = '' then
        Filtro := 'MES = ' + #39 + cmbMes.Text + #39
      else
        Filtro := Filtro + 'AND (MES = ' + #39 + cmbMes.Text + #39 + ')';
    end;

    if (cmbAnio.Text <> '') then
    begin
      if Filtro = '' then
        Filtro := 'ANIO = ' + cmbAnio.Text
      else
        Filtro := Filtro + 'AND (ANIO = ' + cmbAnio.Text + ')';
    end;
...
begin
  ...
  if gListado = PAGOS_EFECTUADOS then //Listado de pagos
  begin
    with TfrmPagos.Create(nil) do
    begin
      if Eliminar(dmQuerys.GetIdActual(gListado)) then
      begin
        gListado := -1;
        ShowList(gOrden, PAGOS_EFECTUADOS);
      end;
      Release;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      Modificar(dmQuerys.GetIdActual(gListado));
      Release;
    end;
    gListado := -1;
    ShowList(gOrden, PAGOS_EFECTUADOS);
...
begin
  ...
  if gListado = PAGOS_EFECTUADOS then
    ...
    begin
      Mostrar(dmQuerys.GetIdActual(gListado), dmQuerys.GetTextActual(gListado, 'PROVEEDOR'));
      Release;
    end;
end;
...
begin
  ...
  case gListado of
    ...
    DESCUENTOS: with TfrmIngDescuento.Create(nil) do //Descuentos
      begin
        R := Agregar(gId);
        if R then
          ShowList(gOrden, DESCUENTOS);
        Release;
      end;
  end;
...
procedure TfrmPrincipal.Generacinexpensasdeptosvacios1Click(Sender: TObject);
var
  M,A: Word;
  R: Boolean;
begin
  ...
  if R then dmDatos.GenerarCostosDeptosVacios(M,A);
end;
...
begin
  ...
  Q.Close;
  Q.SQL.Clear;
  Q.SQL.Add('SELECT DISTINCT ANIO FROM VW_DESCUENTO');
  Q.Open;
  Q.First;
  while not Q.Eof do
  begin
    cmbAnio.Items.Add(Q.FieldByName('ANIO').AsString);
    Q.Next;
  end;

  cmbTipoDescuento.Items.Clear;
  Q.Close;
  Q.SQL.Clear;
  Q.SQL.Add('SELECT DESCRIPCION FROM TIPO_DESCUENTO');
  Q.Open;
  Q.First;
  while not Q.Eof do
  begin
    cmbTipoDescuento.Items.Add(Q.FieldByName('DESCRIPCION').AsString);
    Q.Next;
  end;

  gOrden := 'APENOM, IDDESCUENTO';
  ShowList(gOrden, DESCUENTOS);
end;
...
begin
  ...
  begin
    form := TfrmQRDescuentos.Create(nil);
    form.VistaPrevia(UpperCase(MesComoStr(M)), '03', 'OCUP_CIV', A, 420);
    form.Release;
  end;
end;
...
begin
  ...
  begin
    form := TfrmQRDescuentos.Create(nil);
    form.VistaPrevia(UpperCase(MesComoStr(M)), '03', 'OCUP_MIL', A, 420);
    form.Release;
  end;
end;
...
procedure TfrmPrincipal.Personal1Click(Sender: TObject);
var
...
procedure TfrmPrincipal.Pagosporpagador1Click(Sender: TObject);
var
  FI,FF: TDateTime;
  R: Boolean;
begin
  ...
  with TfrmQRPagosPagador.Create(nil) do
  begin
    VistaPrevia(FI,FF);
    Release;
  end;
...
procedure TfrmPrincipal.Formulariodepagos1Click(Sender: TObject);
var
  M, A: Word;
begin
  if IngresarMes(M, A) then
    frmQRFormularioPago.Execute(UpperCase(MesComoStr(M)), A);
end;

procedure TfrmPrincipal.rgPagadoClick(Sender: TObject);
begin
  rgTipoPago.Enabled := (rgPagado.ItemIndex = 0);
end;

procedure TfrmPrincipal.Personalsuperior1Click(Sender: TObject);
var
  IdEdificio: Integer;
  form: TfrmElecEdificio;
begin
  form := TfrmElecEdificio.Create(nil);
  IdEdificio := form.GetId(1);
  form.Release;
  if IdEdificio <> 0 then
    frmQRTitulares.Execute(1, IdEdificio);
end;

procedure TfrmPrincipal.Personalsubalterno1Click(Sender: TObject);
var
  IdEdificio: Integer;
  form: TfrmElecEdificio;
begin
  form := TfrmElecEdificio.Create(nil);
  IdEdificio := form.GetId(2);
  form.Release;
  if IdEdificio <> 0 then
    frmQRTitulares.Execute(2, IdEdificio);
end;

procedure TfrmPrincipal.Personalcivil1Click(Sender: TObject);
var
  IdEdificio: Integer;
  form: TfrmElecEdificio;
begin
  form := TfrmElecEdificio.Create(nil);
  IdEdificio := form.GetId(3);
  form.Release;
  if IdEdificio <> 0 then
    frmQRTitulares.Execute(3, IdEdificio);
end;

procedure TfrmPrincipal.Remanentes1Click(Sender: TObject);
var
  IdEdificio: Integer;
  form: TfrmElecEdificio;
begin
  form := TfrmElecEdificio.Create(nil);
  IdEdificio := form.GetId(4);
  form.Release;
  if IdEdificio <> 0 then
    frmQRTitularesRemanentes.Execute(4, IdEdificio);
end;

procedure TfrmPrincipal.Gastosadministrativos2Click(Sender: TObject);
var
  FechaInicio, FechaFin: TDateTime;
begin
  if IngresarFechas(FechaInicio, FechaFin) then
    with TfrmEstGastosAdmin.Create(nil) do
    begin
      Mostrar(FechaInicio, FechaFin);
      Release;
    end;
...
procedure TfrmPrincipal.ResumenDECIsMantenimiento1Click(Sender: TObject);
var
  M, A: Word;
begin
  if not IngresarMes(M,A) then Exit;

  with TfrmQRResumenDECIsMant.Create(nil) do
  begin
    VistaPrevia(M,A);
    Release;
  end;
...
procedure TfrmPrincipal.ResumenDECIsServicios1Click(Sender: TObject);
var
  M, A: Word;
begin
  if not IngresarMes(M,A) then Exit;

  with TfrmQRResumenDECIsServ.Create(nil) do
  begin
    VistaPrevia(M,A);
    Release;
  end;
...
procedure TfrmPrincipal.Deudores1Click(Sender: TObject);
begin
  if gListado = TComponent(Sender).Tag then Exit;
  gOrden := 'FECHA_PAGO';
  ShowList(gOrden, DEUDORES);
end;

procedure TfrmPrincipal.Listadodedeudores1Click(Sender: TObject);
var
  FechaInicio, FechaFin: TDateTime;
begin
  if IngresarFechas(FechaInicio, FechaFin) then
    with TfrmQRDeudores.Create(nil) do
    begin
      VistaPrevia(FechaInicio, FechaFin);
      Release;
    end;
...
procedure TfrmPrincipal.Gastosfinancieros1Click(Sender: TObject);
var
  FechaInicio, FechaFin: TDateTime;
begin
  if IngresarFechas(FechaInicio, FechaFin) then
    with TfrmEstGastosFin.Create(nil) do
    begin
      Mostrar(FechaInicio, FechaFin);
      Release;
    end;
