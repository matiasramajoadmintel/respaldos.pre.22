...
* To run this application, Excel 97, 2000, XP, 2007, 2010, 2013 or 2016 should be installed.
* This demo application uses the old BDE Paradox DBDemos tables.
* Upon exit, the BDE should delete any *.lck files. If the BDE throws initialize errors,
then delete all *.lck files in the NET DIR folder manually.
* If you want to use the BDE on Windows Vista or Windows 7 then you will need to
reconfigure the BDE so that it does not attempt to write files to the root directory.
Start BDEAdmin.exe as administrator, go to Configuration >> Drivers >> Native >> Paradox
and choose a location other than the root directory.
* Make sure to add a path to the folder with the scExcelExport.pas file
in the global Library path of Delphi (Tools >> Options >> Delphi Options >> Library)
--------------------------------------------------------------------------------}
...
type
  TFormExcelExport = class(TForm)
    ...
    Panel1: TPanel;
    Panel2: TPanel;
    BitBtnExecute19: TBitBtn;
    ListBoxResults: TListBox;
    BitBtnExecute19b: TBitBtn;
    BitBtnGetExcelDateFormat: TBitBtn;
    EditDateFormat: TEdit;
    ...
    procedure BitBtnExecute19Click(Sender: TObject);
    procedure BitBtnExecute19bClick(Sender: TObject);
    procedure BitBtnGetExcelDateFormatClick(Sender: TObject);
  private
...
begin
  try
    ...
    begin
      ...
      MessageDlg('NO connection with the BDE ' + DatabaseDBDemos.AliasName +' tables could be established !'+#13+#10
      +'Read the comments in the header of the MainExcelExport.pas file to solve this problem.'+#13+#10+E.Message, mtError, [mbOK], 0);
    end;
  end;
...
begin
  ...
  if SameText(QuerySortShipVia.Fields[FieldIndex].FieldName,'SaleDate') then
...
begin
  try
    ...
    scExcelExport1.BorderData.BackColor := clGreen;
    scExcelExport1.BorderData.BackAlternateColor := clYellow;
...
begin
  ...
  intRecords := 100;
  intSheetcount := 1;
  while not TableOrders.IsEmpty do
  begin
    TableOrders.Filtered := False;
    TableOrders.Filter := 'OrderNo > ' + IntToStr(intOrderNo) + ' AND OrderNo < ' + IntToStr(intOrderNo+intRecords);
    TableOrders.Filtered := True;

    scExcelExport1.Dataset:=TableOrders;
    scExcelExport1.WorksheetName:= IntToStr(intSheetcount);
    ...
    intOrderNo := intOrderNo + intRecords;
  end;
...
begin
  ...
  scExcelExport1.ExcelWorkSheet.Name := 'Biolife and Animals'; // Rename worksheet
   ...
   //scExcelExport1.ExcelWorkbook.ActiveSheet = scExcelExport1.ExcelWorkbook.Worksheets[1];
   //scExcelExport1.ExcelWorkbook.Worksheets('Biolife').Select(null, LCID);
   //scExcelExport1.ExcelWorkbook.ActiveSheet.Name = 'New name';
   //scExcelExport1.ExcelWorkbook.ActiveSheet.Next.Select;
...
begin
  try
    ...
    scExcelExport1.BeginRowTitles := 6;
...
//-----------------------------------------------------------------------------
procedure TFormExcelExport.BitBtnExecute19bClick(Sender: TObject);
begin
  ListBoxResults.Items.Add('0001 A   = ' + scExcelExport1.GetColumnCharacters(1));
  ListBoxResults.Items.Add('0015 O   = ' + scExcelExport1.GetColumnCharacters(15));
  ListBoxResults.Items.Add('0026 Z   = ' + scExcelExport1.GetColumnCharacters(26));
  ListBoxResults.Items.Add('0027 AA  = ' + scExcelExport1.GetColumnCharacters(27));
  ListBoxResults.Items.Add('0028 AB  = ' + scExcelExport1.GetColumnCharacters(28));
  ListBoxResults.Items.Add('0150 ET  = ' + scExcelExport1.GetColumnCharacters(150));
  ListBoxResults.Items.Add('0257 IW  = ' + scExcelExport1.GetColumnCharacters(257));
  ListBoxResults.Items.Add('0675 YY  = ' + scExcelExport1.GetColumnCharacters(675));
  ListBoxResults.Items.Add('0676 YZ  = ' + scExcelExport1.GetColumnCharacters(676));
  ListBoxResults.Items.Add('0677 ZA  = ' + scExcelExport1.GetColumnCharacters(677));
  ListBoxResults.Items.Add('0701 ZY  = ' + scExcelExport1.GetColumnCharacters(701));
  ListBoxResults.Items.Add('0702 ZZ  = ' + scExcelExport1.GetColumnCharacters(702));
  ListBoxResults.Items.Add('0703 AAA = ' + scExcelExport1.GetColumnCharacters(703));
  ListBoxResults.Items.Add('0704 AAB = ' + scExcelExport1.GetColumnCharacters(704));
  ListBoxResults.Items.Add('0886 AHB = ' + scExcelExport1.GetColumnCharacters(886));
  ListBoxResults.Items.Add('0951 AJO = ' + scExcelExport1.GetColumnCharacters(0951));
  ListBoxResults.Items.Add('1082 AOP = ' + scExcelExport1.GetColumnCharacters(1082));
  ListBoxResults.Items.Add('1200 ATD = ' + scExcelExport1.GetColumnCharacters(1200));
  ListBoxResults.Items.Add('1313 AXM = ' + scExcelExport1.GetColumnCharacters(1313));
  ListBoxResults.Items.Add('1378 AZZ = ' + scExcelExport1.GetColumnCharacters(1378));
  ListBoxResults.Items.Add('1379 BAA = ' + scExcelExport1.GetColumnCharacters(1379));
  ListBoxResults.Items.Add('1406 BBB = ' + scExcelExport1.GetColumnCharacters(1406));
  ListBoxResults.Items.Add('1749 BOG = ' + scExcelExport1.GetColumnCharacters(1749));
  ListBoxResults.Items.Add('2055 CAA = ' + scExcelExport1.GetColumnCharacters(2055));
  ListBoxResults.Items.Add('2500 CRD = ' + scExcelExport1.GetColumnCharacters(2500));
  ListBoxResults.Items.Add('4028 EXX = ' + scExcelExport1.GetColumnCharacters(4028));
  ListBoxResults.Items.Add('5545 HEG = ' + scExcelExport1.GetColumnCharacters(5545));
end;

//-----------------------------------------------------------------------------
procedure TFormExcelExport.BitBtnExecute19Click(Sender: TObject);
var
  StrCell : String;
  i : Integer;
begin
  try
    scExcelExport1.LoadDefaultProperties;
    scExcelExport1.ConnectTo := ctNewExcel;
    scExcelExport1.DataPipe := dpCustom;
    scExcelExport1.Connect;
    scExcelExport1.ExportDataset();

    try
      i := 10;
      while i < scExcelExport1.ExcelRestrictions.MaxNumberOfColumns + 100 do
      begin
        StrCell := scExcelExport1.GetColumnCharacters(i) + '1';
        scExcelExport1.ExcelWorkSheet.Range[StrCell, StrCell].Value2 := IntToStr(i);
        i := i + 50;
      end;
    except
      on E : Exception do
    ...
    scExcelExport1.ExcelApplication.Visible[scExcelExport1.LCID] := True;
...
//-----------------------------------------------------------------------------
procedure TFormExcelExport.BitBtnGetExcelDateFormatClick(Sender: TObject);
var
  StrDateFormat : String;
begin
  // http://marketfeederpro.com/support/excel-tips/excel-date-time/
  scExcelExport1.LoadDefaultProperties;
  scExcelExport1.ConnectTo := ctNewExcel;
  scExcelExport1.DataPipe := dpCustom;
  scExcelExport1.Connect;
  ...
  StrDateFormat := scExcelExport1.GetDateTimeFormat('dd/mm/yyyy');
  EditDateFormat.Text := StrDateFormat;

  scExcelExport1.ExcelWorkSheet.Range['A1','A1'].Value2 := StrToDate('31/01/2015');
  scExcelExport1.ExcelWorkSheet.Range['A1','A1'].NumberFormat := StrDateFormat;
