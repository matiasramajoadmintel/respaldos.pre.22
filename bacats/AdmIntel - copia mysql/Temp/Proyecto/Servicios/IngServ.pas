...
uses
  ...
  DBTables, DateUtils, ElecOcupante;
...
type
  TfrmIngServ = class(TForm)
    ...
    Label6: TLabel;
    DBEdit1: TDBEdit;
    chkVacios: TDBCheckBox;
    SERVEDIFA_PAGAR_OCUPANTE: TStringField;
    chkPersona: TDBCheckBox;
    PERS: TTable;
    PERSIDPERSONA: TIntegerField;
    PERSNOMBRE: TStringField;
    PERSAPELLIDO: TStringField;
    PERSPersona: TStringField;
    edtPersona: TDBEdit;
    SERVEDIFIDPERSONA: TIntegerField;
    SERVEDIFPersona: TStringField;
    ...
    procedure chkVaciosClick(Sender: TObject);
    procedure chkPersonaMouseUp(Sender: TObject; Button: TMouseButton;
...
function TfrmIngServ.Agregar(var Id: Longword): Boolean;
begin
...
begin
  ...
  begin
    ...
    SERVEDIF.Open;
    SERVEDIF.Append;
    SERVEDIFA_PAGAR_OCUPANTE.Text := 'No';
...
begin
  ...
  if Editando(SERVEDIF) then
    try
      ...
      if chkVacios.Checked then
        SERVEDIFVACIOS.Value := 'Sí'
      else
        SERVEDIFVACIOS.Value := 'No';
      SERVEDIF.Post;
...
procedure TfrmIngServ.PERSCalcFields(DataSet: TDataSet);
begin
 PERSPersona.Text := PERSAPELLIDO.Text + ', ' + PERSNOMBRE.Text;
end;

procedure TfrmIngServ.chkVaciosClick(Sender: TObject);
begin
  if chkVacios.Checked and chkPersona.Checked then
  begin
    chkPersona.Checked := False;
    edtPersona.Enabled := False;
    if not SERVEDIFIDPERSONA.IsNull then
      SERVEDIFIDPERSONA.Clear;
  end;
end;

procedure TfrmIngServ.chkPersonaMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  form: TfrmElecOcupante;
...
begin
  ...
  begin
    ...
    edtPersona.Enabled := True;
  end;

  if chkPersona.Checked then
  begin
    form := TfrmElecOcupante.Create(nil);
    IdPersona := form.GetId(NombrePersona, 1);
    form.Release;
    if IdPersona <> 0 then
      SERVEDIFIDPERSONA.Value := IdPersona
    else
      SERVEDIFA_PAGAR_OCUPANTE.Value := 'No';
  end
  else
  begin
    if not SERVEDIFIDPERSONA.IsNull then
